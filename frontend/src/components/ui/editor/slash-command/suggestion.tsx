...
const getAllItems = (options: SuggestionOptions): SlashItem[] => [
  {
    title: i18n.t("editor.blocks.text"),
    icon: Pilcrow,
    command: ({ editor, range }) => editor.chain().focus().deleteRange(range).setParagraph().run(),
  },
  {
    title: i18n.t("editor.blocks.heading1"),
    icon: Heading1,
    command: ({ editor, range }) => editor.chain().focus().deleteRange(range).setHeading({ level: 1 }).run(),
  },
  {
    title: i18n.t("editor.blocks.heading2"),
    icon: Heading2,
    command: ({ editor, range }) => editor.chain().focus().deleteRange(range).setHeading({ level: 2 }).run(),
  },
  {
    title: i18n.t("editor.blocks.heading3"),
    icon: Heading3,
    command: ({ editor, range }) => editor.chain().focus().deleteRange(range).setHeading({ level: 3 }).run(),
  },
  {
    title: i18n.t("editor.blocks.bulletList"),
    icon: List,
    command: ({ editor, range }) => editor.chain().focus().deleteRange(range).toggleBulletList().run(),
  },
  {
    title: i18n.t("editor.blocks.orderedList"),
    icon: ListOrdered,
    command: ({ editor, range }) => editor.chain().focus().deleteRange(range).toggleOrderedList().run(),
  },
  {
    title: i18n.t("editor.actions.image"),
    icon: Image,
  ...
  {
    title: i18n.t("editor.actions.table"),
    icon: Table,
    command: ({ editor, range }) =>
      editor.chain().focus().deleteRange(range).insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run(),
  },
  {
    title: i18n.t("editor.blocks.quote"),
    icon: Quote,
    command: ({ editor, range }) => editor.chain().focus().deleteRange(range).toggleBlockquote().run(),
  },
  {
    title: i18n.t("editor.blocks.codeBlock"),
    icon: Code,
