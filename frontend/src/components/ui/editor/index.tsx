...
export type EditorProps = {
  ...
  onRequestFile?: FileInsertHandler;
...
const blockOptions: Array<{ value: BlockType; labelKey: string }> = [
  { value: "paragraph", labelKey: "editor.blocks.text" },
  { value: "heading1", labelKey: "editor.blocks.heading1" },
  { value: "heading2", labelKey: "editor.blocks.heading2" },
  { value: "heading3", labelKey: "editor.blocks.heading3" },
  { value: "bulletList", labelKey: "editor.blocks.bulletList" },
  { value: "orderedList", labelKey: "editor.blocks.orderedList" },
  { value: "blockquote", labelKey: "editor.blocks.quote" },
  { value: "codeBlock", labelKey: "editor.blocks.codeBlock" },
];
...
export function Editor({
  ...
  useEffect(() => {
    ...
    return () => {
      // eslint-disable-next-line react-hooks/exhaustive-deps -- ref copied to local var before use
      const objectUrls = objectUrlByUploadIdRef.current;
      // eslint-disable-next-line react-hooks/exhaustive-deps -- ref copied to local var before use
      const expectedBlobs = expectedBlobByUploadIdRef.current;
  ...
  const fileToDataUrl = (file: File): Promise<string> =>
    new Promise<string>((resolve, reject) => {
      ...
      reader.onerror = () => reject(new Error(i18n.t("editor.errors.readFailed")));
    ...
    try {
      ...
      if (!resolved?.src) {
        finalizeImageUpload(uploadId, (attrs) => ({
          ...attrs,
          uploading: false,
          uploadError: i18n.t("editor.errors.uploadFailed"),
        }));
      ...
      if (!preloaded) {
        finalizeImageUpload(uploadId, (attrs) => ({
          ...attrs,
          uploading: false,
          uploadError: i18n.t("editor.errors.previewFailed"),
        }));
        cleanupUpload(uploadId, { revokeBlob: false });
        return;
      }
      ...
      finalizeImageUpload(uploadId, (attrs) => ({
        ...attrs,
        uploading: false,
        uploadError: error instanceof Error ? error.message : i18n.t("editor.errors.uploadFailed"),
      }));
      cleanupUpload(uploadId, { revokeBlob: false });
    }
  };
  ...
  const inlineActions: MenuAction[] = [
    {
      label: i18n.t("editor.actions.bold"),
      icon: Bold,
    ...
    {
      label: i18n.t("editor.actions.italic"),
      icon: Italic,
    ...
    {
      label: i18n.t("editor.actions.underline"),
      icon: UnderlineIcon,
    ...
    {
      label: i18n.t("editor.actions.strikethrough"),
      icon: Strikethrough,
    ...
    {
      label: i18n.t("editor.actions.code"),
      icon: Code,
    ...
    {
      label: i18n.t("editor.actions.removeFormatting"),
    ...
    <div {...props} className={cn("", className)}>
      ...
      >
        <div className="flex flex-col gap-1">
          <div className="border-border bg-popover flex flex-nowrap items-center gap-0.5 overflow-x-auto rounded-md border p-1 shadow-sm whitespace-nowrap">
            {!isInTable ? (
              <div className="group/native-select relative w-fit">
                <select
                  ...
                  disabled={disabled}
                  aria-label={i18n.t("editor.aria.blockStyle")}
            ...
            {renderIconButton({
              label: i18n.t("editor.actions.link"),
              icon: LinkIcon,
            ...
            {isOnImage ? (
              <button
                type="button"
                aria-label={i18n.t("editor.aria.imageAltText")}
                title={i18n.t("editor.aria.imageAltText")}
          ...
          {showLinkInput ? (
            ...
            >
              ...
              />
              {renderIconButton({
                label: i18n.t("editor.actions.setLink"),
                icon: Check,
                onClick: applyLink,
                disabled: disabled || !linkUrl.trim(),
              })}
              {renderIconButton({
                label: i18n.t("editor.actions.removeLink"),
                icon: X,
          ...
          {showAltInput && isOnImage ? (
            ...
            >
              <input
                id="image-alt"
                type="text"
                placeholder={i18n.t("editor.placeholders.describeImage")}
                value={imageAltText}
              ...
              />
              {renderIconButton({
                label: i18n.t("editor.actions.saveAltText"),
                icon: Check,
                onClick: applyImageAlt,
                disabled,
              })}
              {renderIconButton({
                label: i18n.t("editor.actions.removeAltText"),
                icon: X,
          ...
          {showTableActions && isInTable ? (
            ...
            >
              ...
              {renderIconButton({
                label: i18n.t("editor.actions.removeRow"),
                icon: Minus,
                onClick: removeRow,
                disabled,
              })}
              ...
              {renderIconButton({
                label: i18n.t("editor.actions.removeColumn"),
                icon: Minus,
      ...
      {showToolbar && editor ? (
        <div className="flex flex-nowrap items-center gap-0.5 overflow-x-auto border-b border-border bg-muted/30 px-2 py-1 whitespace-nowrap rounded-t-md">
          ...
          {/* Headings */}
          {renderIconButton({
            label: i18n.t("editor.blocks.heading1"),
            icon: Heading1,
            onClick: () => editor.chain().focus().toggleHeading({ level: 1 }).run(),
            disabled,
            toggle: true,
            pressed: activeState.blockType === "heading1",
          })}
          {renderIconButton({
            label: i18n.t("editor.blocks.heading2"),
            icon: Heading2,
            onClick: () => editor.chain().focus().toggleHeading({ level: 2 }).run(),
            disabled,
            toggle: true,
            pressed: activeState.blockType === "heading2",
          })}
          {renderIconButton({
            label: i18n.t("editor.blocks.quote"),
            icon: Quote,
            onClick: () => editor.chain().focus().toggleBlockquote().run(),
            disabled,
            toggle: true,
            pressed: activeState.blockType === "blockquote",
          })}
          <span className="bg-border mx-0.5 h-4 w-px" aria-hidden="true" />
          {/* Lists */}
          {renderIconButton({
            label: i18n.t("editor.blocks.bulletList"),
            icon: List,
            onClick: () => editor.chain().focus().toggleBulletList().run(),
            disabled,
            toggle: true,
            pressed: activeState.blockType === "bulletList",
          })}
          {renderIconButton({
            label: i18n.t("editor.blocks.orderedList"),
            icon: ListOrdered,
            onClick: () => editor.chain().focus().toggleOrderedList().run(),
            disabled,
            toggle: true,
            pressed: activeState.blockType === "orderedList",
          })}
          <span className="bg-border mx-0.5 h-4 w-px" aria-hidden="true" />
          {/* Insert group */}
          {renderIconButton({
          ...
          {enableImages && onRequestImage
            ? renderIconButton({
                label: i18n.t("editor.actions.image"),
                icon: ImageIcon,
                onClick: () => {
                  if (onRequestImage) {
                    const result = onRequestImage({ editor, range: { from: 0, to: 0 } });
                    Promise.resolve(result).then((resolved) => {
                      if (resolved && resolved.kind === "url") {
                        editor.chain().focus().setImage({ src: resolved.src, alt: resolved.alt }).run();
                      } else if (resolved && resolved.kind === "file") {
                        const fileResult = resolved as ImagePickerFileResult;
                        void insertLocalImageFile(fileResult.file, "slash", {
                          alt: fileResult.alt,
                        });
                      }
                    });
          ...
          {onRequestFile
            ? renderIconButton({
                label: i18n.t("editor.actions.attachFile"),
                icon: Paperclip,
                onClick: () => {
                  if (onRequestFile) {
                    void onRequestFile();
          ...
          {/* History */}
          {renderIconButton({
            label: i18n.t("editor.actions.undo"),
            icon: Undo2,
            onClick: () => editor.chain().focus().undo().run(),
            disabled: disabled || !editor.can().undo(),
          })}
          {renderIconButton({
            label: i18n.t("editor.actions.redo"),
            icon: Redo2,
