      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder={t("clinicNotes.searchPlaceholder")}
            className="ps-8"
          />
        </div>
        <Button onClick={handleCreate}>
          <PlusIcon className="size-4" />
          {t("clinicNotes.newNote")}
        </Button>
      </div>
          ...
          {notes.map((note) => (
            <NoteCard
              ...
              getAttachmentUrl={(fileId) =>
                ClinicNoteService.getAttachmentDownloadUrl(note.id, fileId)
              }
              getAttachmentPreviewUrl={(fileId) =>
                ClinicNoteService.getAttachmentPreviewUrl(note.id, fileId)
              }
            />
          ))}
          </div>
